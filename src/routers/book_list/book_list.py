...
@router.get('/user/{user_id}', response_model=list[BookListBaseSchema])
async def get_user_book_lists(user_id: int) -> list[BookList]:
    """Return all book lists of a user."""
    return BookList.list([BookList.user_id == user_id])
